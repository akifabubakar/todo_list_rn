import {
  View,
  Text,
  SafeAreaView,
  TouchableOpacity,
  StyleSheet,
  ScrollView,
  TextInput,
  Alert,
} from 'react-native';
import React, { useState } from 'react';
import { useTasksStore } from '../services/zustand/store';
import { useForm, Controller } from 'react-hook-form';
import DatePicker from 'react-native-date-picker';
import { toast } from '@backpackapp-io/react-native-toast';
import dayjs from 'dayjs';

function Header({ onDelete }) {
  return (
    <View
      style={{
        paddingHorizontal: 10,
        flexDirection: 'row',
        alignItems: 'flex-end',
        justifyContent: 'space-between',
        paddingBottom: 20,
      }}>
      <Text style={{ color: 'white', fontSize: 32, fontWeight: 'bold' }}>
        Update Task
      </Text>

      <TouchableOpacity onPress={onDelete}>
        <Text style={{ color: '#FF9D9D', fontSize: 24, fontWeight: 'bold' }}>
          Delete
        </Text>
      </TouchableOpacity>
    </View>
  );
}

function Input(props) {
  return (
    <TextInput
      {...props}
      style={{
        backgroundColor: 'white',
        paddingVertical: 15,
        paddingHorizontal: 10,
        borderRadius: 10,
        paddingTop: 15,
        fontWeight: 'bold',
      }}
    />
  );
}

export default function UpdateTaskScreen({ route, navigation }) {
  const updateTask = useTasksStore(state => state.updateTask);
  const deleteTask = useTasksStore(state => state.deleteTask);

  const [picker, setpicker] = useState(false);

  const current = route.params;

  const [dateString, setDateString] = useState(() => {
    let today = dayjs().endOf('day').toISOString();

    return dayjs(current?.date).toISOString() === today
      ? 'Today'
      : new Date(current?.date).toLocaleDateString();
  });

  const {
    control,
    handleSubmit,
    formState: { dirtyFields },
    reset,
    setValue,
    getValues,
    watch,
  } = useForm({
    defaultValues: {
      _id: current?._id,
      title: current?.title ?? '',
      description: current?.description ?? '',
      date: current?.date ?? new Date().toISOString(),
      important: current?.important ?? false,
    },
  });

  const important = watch('important');

  const onSubmit = data => {
    let end = dayjs(data.date).endOf('day');

    updateTask({
      ...data,
      date: end,
    });
    setDateString('Today');
    reset();
    navigation.goBack();
    toast('Task updated');
  };

  const onSelectDate = date => {
    let today = dayjs().endOf('day').toISOString();
    let selected = dayjs(date).endOf('day').toISOString();

    selected === today
      ? setDateString('Today')
      : setDateString(new Date(selected).toLocaleDateString());

    setValue('date', date, { shouldDirty: true });
    setpicker(false);
  };

  const onDelete = () => {
    Alert.alert('', 'Delete this task?', [
      {
        text: 'Cancel',
        onPress: () => {},
        style: 'cancel',
      },
      {
        text: 'OK',
        onPress: () => {
          deleteTask({ _id: current?._id });
          navigation.goBack();
          toast('Task deleted');
        },
      },
    ]);
  };

  return (
    <SafeAreaView style={{ backgroundColor: '#35395B', flex: 1 }}>
      <Header onDelete={onDelete} />
      <ScrollView
        style={{
          flex: 1,
          paddingHorizontal: 10,
        }}>
        {/* Title */}
        <Controller
          control={control}
          rules={{
            required: true,
          }}
          render={({ field: { onChange, onBlur, value } }) => (
            <Input
              placeholder="Title"
              onBlur={onBlur}
              onChangeText={onChange}
              value={value}
            />
          )}
          name="title"
        />

        <View style={{ paddingVertical: 5 }} />

        {/* Description */}
        <Controller
          control={control}
          rules={{
            required: false,
          }}
          render={({ field: { onChange, onBlur, value } }) => (
            <Input
              placeholder="Description"
              onBlur={onBlur}
              onChangeText={onChange}
              value={value}
              multiline
              numberOfLines={4}
            />
          )}
          name="description"
        />

        <View style={{ paddingVertical: 5 }} />

        {/* Date Field */}
        <View
          style={{
            backgroundColor: 'white',
            borderRadius: 10,
            flexDirection: 'row',
            overflow: 'hidden',
          }}>
          <View
            style={{ flex: 0.5, paddingHorizontal: 10, paddingVertical: 15 }}>
            <Text style={{ fontWeight: 'bold' }}>Date</Text>
          </View>
          <TouchableOpacity
            onPress={() => {
              setpicker(true);
            }}
            style={{
              flex: 0.5,
              alignItems: 'center',
              backgroundColor: '#397CFF',
              paddingHorizontal: 10,
              paddingVertical: 15,
            }}>
            <Text style={{ color: 'white', fontWeight: 'bold' }}>
              {dateString}
            </Text>
          </TouchableOpacity>
        </View>

        <View style={{ paddingVertical: 5 }} />

        {/* Important Checkbox */}
        <View
          style={{
            flexDirection: 'row',
            alignItems: 'center',
            justifyContent: 'flex-end',
          }}>
          <Text
            style={{ color: 'white', fontWeight: 'bold', paddingRight: 10 }}>
            Important
          </Text>

          <TouchableOpacity
            onPress={() => {
              setValue('important', !important, { shouldDirty: true });
            }}
            style={!important ? styles.checkbox : styles.tickCheckbox}
          />
        </View>
      </ScrollView>

      {/* Clear & Add Button */}
      <View
        style={{
          flexDirection: 'row',
          justifyContent: 'space-evenly',
          paddingVertical: 20,
        }}>
        <TouchableOpacity
          onPress={() => {
            setDateString('Today');
            reset();
            navigation.goBack();
          }}
          style={{
            backgroundColor: '#FF9D9D',
            paddingVertical: 15,
            paddingHorizontal: 10,
            flex: 0.4,
            borderRadius: 10,
          }}>
          <Text
            style={{ color: 'white', fontWeight: 'bold', textAlign: 'center' }}>
            Cancel
          </Text>
        </TouchableOpacity>

        <TouchableOpacity
          disabled={Object.keys(dirtyFields).length === 0}
          onPress={handleSubmit(onSubmit)}
          style={{
            backgroundColor:
              Object.keys(dirtyFields).length === 0 ? '#CACACA' : '#397CFF',
            paddingVertical: 15,
            paddingHorizontal: 10,
            flex: 0.4,
            borderRadius: 10,
          }}>
          <Text
            style={{
              color: 'white',
              fontWeight: 'bold',
              textAlign: 'center',
            }}>
            Update
          </Text>
        </TouchableOpacity>
      </View>

      <DatePicker
        modal
        open={picker}
        date={new Date(getValues('date'))}
        onConfirm={onSelectDate}
        onCancel={() => {
          setpicker(false);
        }}
        mode="date"
      />
    </SafeAreaView>
  );
}

const styles = StyleSheet.create({
  checkbox: {
    backgroundColor: '#EAEAEA',
    width: 40,
    height: 40,
    borderRadius: 5,
    borderColor: 'grey',
    borderWidth: 1,
  },
  tickCheckbox: {
    backgroundColor: '#99BBFF',
    width: 40,
    height: 40,
    borderRadius: 5,
    borderColor: '#397CFF',
    borderWidth: 1,
  },
});
